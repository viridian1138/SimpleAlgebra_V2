...
import simplealgebra.MultRightMutator;
...
public class MultRightMutatorType<T extends Elem<T,?>> extends HGAtomTypeBase {
	...
	public Object make(HGPersistentHandle handle, LazyRef<HGHandle[]> targetSet,
		...
		if( name == null )
			throw( new RuntimeException( "Failed" ) );
		return( new MultRightMutator<T>( elem , name ) );
	}
	...
	public HGPersistentHandle store(Object instance) {
		MultRightMutator<T> oid = (MultRightMutator<T>)( instance );
	...
	{
		MultRightMutatorType<?> type = new MultRightMutatorType();
		HGPersistentHandle typeHandle = graph.getHandleFactory().makeHandle();
		graph.getTypeSystem().addPredefinedType( typeHandle , type , MultRightMutator.class );
	}
