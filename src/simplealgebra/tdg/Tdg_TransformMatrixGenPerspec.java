 ...
 */
public class Tdg_TransformMatrixGenPerspec<U extends NumDimensions, A extends Ord<U>, R extends Elem<R,?>, S extends ElemFactory<R,S>>
{
	 ...
	 */
	public Tdg_TransformMatrixGenPerspec( final GeometricAlgebraMultivectorElemFactory<U,A,R,S> _gaFactory , final SquareMatrixElemFactory<U,R,S> _matrixFactory )
	{
	...
	public SquareMatrixElem<U,R,S> rotationMatrix( final BigInteger axisA , final BigInteger axisB , final R angleRadians , final int numIter ) throws DimensionMismatchException
	{
		...
		final BigInteger dimsH = dim.getVal();
		final BigInteger dimsL = dimsH.subtract( BigInteger.ONE );
		
		if( dimsL.compareTo( axisA ) <= 0 )
		{
			throw( new DimensionMismatchException() );
		}
		
		if( dimsL.compareTo( axisB ) <= 0 )
		{
			throw( new DimensionMismatchException() );
		}
		...
		for( BigInteger cnt = BigInteger.ZERO ; cnt.compareTo( dimsH ) < 0 ; cnt = cnt.add( BigInteger.ONE ) )
		{
	...
	public SquareMatrixElem<U,R,S> scalingMatrix( final GeometricAlgebraMultivectorElem<U,A,R,S> scale , final boolean strictScale )
	{
		...
		final BigInteger udim = getMatrixFactory().getDim().getVal()
				.subtract( BigInteger.ONE );
		ret.setVal(udim, udim, getMatrixFactory().getFac().identity() );
	 ...
	 * @param scale Vector containing the translation
	 * @return The generated translation matrix
	 */
	public SquareMatrixElem<U,R,S> translationMatrix( final GeometricAlgebraMultivectorElem<U,A,R,S> scale )
	{
		final SquareMatrixElem<U,R,S> ret = matrixFactory.identity();
		
		final BigInteger translationRow = matrixFactory.getDim().getVal().subtract( BigInteger.ONE );
		...
		for( final Entry<HashSet<BigInteger>, R> v : scale.getEntrySet() )
		{
			...
			if( key.size() == 1 )
			{
				for( final BigInteger kv : key )
				{
					ret.setVal(translationRow, kv, val);
				}
			}
		}
