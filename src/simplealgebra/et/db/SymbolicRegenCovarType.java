...
/**
 * HyperGraph ( <A href="http://hypergraphdb.org">http://hypergraphdb.org</A> ) type for storing SymbolicRegenCovar instances.
 * 
 ...
 *
 */
public class SymbolicRegenCovarType<Z extends Object, R extends Elem<R,?>, S extends ElemFactory<R,S>> extends HGAtomTypeBase {

	
	/**
	 * Constructs the type instance.
	 */
	public SymbolicRegenCovarType() {
	}
	...
	public HGPersistentHandle store(Object instance) {
		SymbolicRegenCovar<Z,R,S> oid = (SymbolicRegenCovar<Z,R,S>)( instance );
		HGHandle elemHandle = hg.assertAtom(graph, oid.getElem() );
		HGHandle facHandle = hg.assertAtom(graph, oid.getFac().getFac() );
		HGHandle contravariantHandle = graph.add( oid.getNewCovar() ); // hg.assertAtom(graph, oid.getCovariantIndices() ); !!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	{
		SymbolicRegenCovarType<?,?,?> type = new SymbolicRegenCovarType();
		HGPersistentHandle typeHandle = graph.getHandleFactory().makeHandle();
		graph.getTypeSystem().addPredefinedType( typeHandle , type , SymbolicRegenCovar.class );
	}
