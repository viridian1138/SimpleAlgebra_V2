...
/**
 * HyperGraph ( <A href="http://hypergraphdb.org">http://hypergraphdb.org</A> ) type for storing SymbolicRegenContravar instances.
 * 
 ...
 *
 */
public class SymbolicRegenContravarType<Z extends Object, R extends Elem<R,?>, S extends ElemFactory<R,S>> extends HGAtomTypeBase {

	
	/**
	 * Constructs the type instance.
	 */
	public SymbolicRegenContravarType() {
	}
	...
	public HGPersistentHandle store(Object instance) {
		SymbolicRegenContravar<Z,R,S> oid = (SymbolicRegenContravar<Z,R,S>)( instance );
		HGHandle elemHandle = hg.assertAtom(graph, oid.getElem() );
		HGHandle facHandle = hg.assertAtom(graph, oid.getFac().getFac() );
		HGHandle contravariantHandle = graph.add( oid.getNewContravar() ); // hg.assertAtom(graph, oid.getCovariantIndices() ); !!!!!!!!!!!!!!!!!!!!!!!!!!
		HGPersistentHandle[] hn = { elemHandle.getPersistent() , facHandle.getPersistent() , 
				contravariantHandle.getPersistent() };
	...
	{
		SymbolicRegenContravarType<?,?,?> type = new SymbolicRegenContravarType();
		HGPersistentHandle typeHandle = graph.getHandleFactory().makeHandle();
		graph.getTypeSystem().addPredefinedType( typeHandle , type , SymbolicRegenContravar.class );
	}
