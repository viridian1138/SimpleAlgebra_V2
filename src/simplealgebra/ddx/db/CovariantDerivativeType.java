...
/**
 * HyperGraph ( <A href="http://hypergraphdb.org">http://hypergraphdb.org</A> ) type for storing CovariantDerivative instances.
 * 
 ...
 */
public class CovariantDerivativeType<Z extends Object, U extends NumDimensions, R extends Elem<R,?>, S extends ElemFactory<R,S>, K extends Elem<?,?>> extends HGAtomTypeBase {

	
	/**
	 * Constructs the type instance.
	 */
	public CovariantDerivativeType() {
	}
	...
	public Object make(HGPersistentHandle handle, LazyRef<HGHandle[]> targetSet,
		...
		Z derivativeIndex = graph.get( layout[ 1 ] );
		...
		DerivativeRemap<Z,R,S> remap = graph.get( layout[ 7 ] );
		if( fac == null )
		...
		final CovariantDerivative<Z,U,R,S,K> et = new CovariantDerivative<Z,U,R,S,K>( param );
		return( et );
	}
	...
	public HGPersistentHandle store(Object instance) {
		CovariantDerivative<Z,U,R,S,K> oid = (CovariantDerivative<Z,U,R,S,K>)( instance );
	...
	{
		CovariantDerivativeType<?,?,?,?,?> type = new CovariantDerivativeType();
		HGPersistentHandle typeHandle = graph.getHandleFactory().makeHandle();
		graph.getTypeSystem().addPredefinedType( typeHandle , type , CovariantDerivative.class );
	}
