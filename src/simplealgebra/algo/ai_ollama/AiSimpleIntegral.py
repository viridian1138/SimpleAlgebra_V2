...
"""
Requests for Ollama AI to determine and verify the integral


Args:

expressionStr -- The expression for which to calculate the integral
wrt -- String containing the variable for which to calculate the integral.
...
"""
def verifyIntegrate( expressionStrIn , wrt , f ) : 

    retryCount = 0

    expressionStr = "( " + expressionStrIn + " ) * d" + wrt

    while retryCount < 10 :

        promptStrA = "Format the answer as a SymPy expression.  Calculus Indefinite integral: " + expressionStr
        ...
        if resultStr is not None : 
            ...
            promptStrA = "Answer Yes/No, does the calculus indefinite integral of " + expressionStr + " yield the answer ( " + resultStr + " ) ? "
...
"""
Runs the overall AI integral algorithm
...
def overallRun( args , f ) : 
    ...
    resultStr = verifyIntegrate( expressionStr , wrt , f )
...
if __name__ == "__main__" :
    ...
    parser = argparse.ArgumentParser(description='AI Simple Integral')
    parser.add_argument('--expr', type=str, default="( 4 * x ** 3 + 5 * x ** 2 + 7 * x ) * dx" ,  
        help='Expression to be parsed.  Must be understandable by the AI.')
    parser.add_argument('--wrt', type=str, default="x" ,  
        help='Variable with respect to which to take the integral.  Since this is processed by AI, single-letter variables are likely preferred.')
