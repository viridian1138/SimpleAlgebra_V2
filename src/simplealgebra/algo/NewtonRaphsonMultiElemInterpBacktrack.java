 ...
 * </math>
 *
 * Includes an interpolating backtracking method.
 * 
...
public class NewtonRaphsonMultiElemInterpBacktrack<U extends NumDimensions, R extends Elem<R,?>, S extends ElemFactory<R,S>>
					...
					throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		for( final ArrayList<? extends Elem<?,?>> withRespectTo : _param.getWithRespectTos() )
		{
			...
			for( final HashSet<BigInteger> key2A : _param.getFunctions().getKeySet() )
			{
	...
	protected void performIteration() throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		final GeometricAlgebraMultivectorElem<U,GeometricAlgebraOrd<U>,R,S> inVal = param.getIterationValue();
		...
		else
		{
			...
			while( ( cnt > 0 ) && ( nextValues == null ) )
			{
				...
				param.setIterationValue( inVal.add( iterationOffset ) );
	...
	@Override
	public NewtonRaphsonMultiElemInterpBacktrack<U,R,S> cloneThread( final DescentAlgorithmMultiElemInputParamCallbacks<U,R,S> _param, BigInteger threadIndex )
	{
		return( new NewtonRaphsonMultiElemInterpBacktrack<U,R,S>( this , _param , threadIndex ) );
	}
	
	
	@Override
	public NewtonRaphsonMultiElemInterpBacktrack<U,R,S> cloneThreadCached(  
			final DescentAlgorithmMultiElemInputParamCallbacks<U,R,S> _param, 
			final CloneThreadCache<GeometricAlgebraMultivectorElem<U, GeometricAlgebraOrd<U>, SymbolicElem<SymbolicElem<R, S>, SymbolicElemFactory<R, S>>, SymbolicElemFactory<SymbolicElem<R, S>, SymbolicElemFactory<R, S>>>,GeometricAlgebraMultivectorElemFactory<U, GeometricAlgebraOrd<U>, SymbolicElem<SymbolicElem<R, S>, SymbolicElemFactory<R, S>>, SymbolicElemFactory<SymbolicElem<R, S>, SymbolicElemFactory<R, S>>>> cache , 
			CloneThreadCache<?,?> cacheImplicit ,
			final BigInteger threadIndex )
	{
		return( new NewtonRaphsonMultiElemInterpBacktrack<U,R,S>( this , _param , cache , cacheImplicit, threadIndex ) );
	}
