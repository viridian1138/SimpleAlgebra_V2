 ...
 */
public class SymbolicPowL<R extends Elem<R,?>, S extends ElemFactory<R,S>> extends SymbolicElem<R,S> 
{
	 ...
	 */
	public SymbolicPowL( SymbolicElem<R,S> _elemA , SymbolicElem<R,S> _elemB , S _fac , int _ivalExp , int _ivalLn )
	{
		super( _fac );
		elemA = _elemA;
		elemB = _elemB;
	 ...
	 */
	public SymbolicPowL( SymbolicElem<R,S> _elemA , SymbolicElem<R,S> _elemB , S _fac , int _ivalExp , int _ivalLn , DroolsSession ds )
	{
	...
	public R eval( HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ) throws NotInvertibleException, MultiplicativeDistributionRequiredException {
		return( elemA.eval( implicitSpace ).powL( elemB.eval( implicitSpace ) , ivalExp , ivalLn ) );
	}
	...
	public R evalCached( HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ,
		...
		final R ret = elemA.evalCached( implicitSpace , cache ).powL( elemB.evalCached( implicitSpace , cache ) , ivalExp , ivalLn );
		cache.put( key , ret );
		return( ret );
	}
	...
	public R evalPartialDerivative( ArrayList<? extends Elem<?,?>> withRespectTo , HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ) throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		{
			SymbolicElem<R,S> derivTerm = elemB.mult( elemA.ln(ivalExp, ivalLn) );
			final R expV = elemA.eval( implicitSpace ).powL( elemB.eval( implicitSpace ) , ivalExp , ivalLn );
			final R dv = derivTerm.evalPartialDerivative( withRespectTo , implicitSpace );
			return( expV.mult( dv ) );
		}
		else
		{
			return( super.powL( elemB , ivalExp , ivalLn ).evalPartialDerivative(withRespectTo, implicitSpace) );
		}
	}
			...
			HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace , HashMap<SCacheKey<R, S>, R> cache ) throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		{
			SymbolicElem<R,S> derivTerm = elemB.mult( elemA.ln(ivalExp, ivalLn) );
			final R expV = elemA.evalCached( implicitSpace , cache ).powL( elemB.evalCached( implicitSpace , cache ) , ivalExp , ivalLn );
		...
		{
			final R ret = super.powL( elemB , ivalExp , ivalLn ).evalPartialDerivativeCached(withRespectTo, implicitSpace, cache);
			cache.put(key, ret);
			return( ret );
		}
	}
	...
	@Override
	public SymbolicPowL<R,S> cloneThread( final BigInteger threadIndex )
	{
		...
		{
			return( new SymbolicPowL<R,S>( elemsA , elemsB , facs , ivalExp , ivalLn ) );
		}
		return( this );
	}
	...
	public SymbolicElem<R,S> cloneThreadCached(
		...
		if( ( elemsA != elemA ) || ( elemsB != elemB ) || ( facs != fac ) )
		{
			final SymbolicPowL<R,S> rtmp = new SymbolicPowL<R,S>( elemsA , elemsB , facs , ivalExp , ivalLn );
			cache.put(this, rtmp);
			return( rtmp );
		}
		cache.put(this, this);
	...
	public String writeDesc( WriteElemCache<SymbolicElem<R,S>,SymbolicElemFactory<R,S>> cache , PrintStream ps )
	{
		...
		if( st == null )
		{
			...
			cache.put(this, st);
			ps.print( SymbolicPowL.class.getSimpleName() );
	...
	{
		ps.print( "<mrow><mi>powl</mi><mo>&ApplyFunction;</mo>" );
