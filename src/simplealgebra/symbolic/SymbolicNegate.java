...
public class SymbolicNegate<R extends Elem<R,?>, S extends ElemFactory<R,S>> extends SymbolicElem<R,S> 
{
	...
	/**
	 * Constructs the negation for use in a Drools ( <A href="http://drools.org">http://drools.org</A> ) session.
	 * 
	 * @param _elem The enclosed elem.
	 * @param _fac The enclosed factory.
			...
			HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace , HashMap<SCacheKey<R, S>, R> cache ) throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		final R ret = elem.evalPartialDerivativeCached( withRespectTo , implicitSpace , cache ).negate();
		cache.put(key, ret);
		return( ret );
	}
	...
	public SymbolicElem<R,S> cloneThreadCached(
		...
		if( ( elems != elem ) || ( facs != fac ) )
		{
			final SymbolicNegate<R,S> rtmp = new SymbolicNegate<R,S>( elems , facs );
			cache.put(this, rtmp);
			return( rtmp );
		}
		cache.put(this, this);
	...
	public String writeDesc( WriteElemCache<SymbolicElem<R,S>,SymbolicElemFactory<R,S>> cache , PrintStream ps )
	{
		...
		if( st == null )
		{
			...
			cache.put(this, st);
			ps.print( SymbolicNegate.class.getSimpleName() );
