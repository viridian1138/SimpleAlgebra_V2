...
public class SymbolicInvertRight<R extends Elem<R,?>, S extends ElemFactory<R,S>> extends SymbolicElem<R,S> 
{
	...
	public R evalCached( HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ,
		...
		final R ret = elem.evalCached( implicitSpace , cache ).invertRight();
		cache.put( key , ret );
		return( ret );
	}
	...
	public SymbolicElem<R,S> cloneThreadCached(
		...
		if( ( elems != elem ) || ( facs != fac ) )
		{
			final SymbolicInvertRight<R,S> rtmp = new SymbolicInvertRight<R,S>( elems , facs );
			cache.put(this, rtmp);
			return( rtmp );
		}
		cache.put(this, this);
	...
	public String writeDesc( WriteElemCache<SymbolicElem<R,S>,SymbolicElemFactory<R,S>> cache , PrintStream ps )
	{
		...
		if( st == null )
		{
			...
			cache.put(this, st);
			ps.print( SymbolicInvertRight.class.getSimpleName() );
