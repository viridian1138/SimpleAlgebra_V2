...
/**
 * A symbolic elem for the hyperbolic sine function.
 * 
 ...
 */
public class SymbolicSinh<R extends Elem<R,?>, S extends ElemFactory<R,S>> extends SymbolicElem<R,S> 
{
	 ...
	 */
	public SymbolicSinh( SymbolicElem<R,S> _elem , S _fac , int _ival )
	{
		super( _fac );
		elem = _elem;
		ival = _ival;
	 ...
	 */
	public SymbolicSinh( SymbolicElem<R,S> _elem , S _fac , int _ival , DroolsSession ds )
	{
	...
	public R evalPartialDerivative( ArrayList<? extends Elem<?,?>> withRespectTo , HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ) throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		{
			final R sinV = elem.cosh( ival ).eval( implicitSpace );
			final R dv = elem.evalPartialDerivative( withRespectTo , implicitSpace );
			return( sinV.mult( dv ) );
		}
		else
		{
			return( super.sinh( ival ).evalPartialDerivative(withRespectTo, implicitSpace) );
		}
	}
			...
			HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace , HashMap<SCacheKey<R, S>, R> cache ) throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		if( ( this.getFac().isMultCommutative() ) && ( this.getFac().isMultAssociative() ) )
		{
			final R sinV = elem.cosh( ival ).evalCached( implicitSpace , cache );
			final R dv = elem.evalPartialDerivativeCached( withRespectTo , implicitSpace , cache );
			final R ret = sinV.mult( dv );
			cache.put(key, ret);
			return( ret );
		}
		else
		{
			final R ret = super.sinh( ival ).evalPartialDerivativeCached(withRespectTo, implicitSpace, cache);
			cache.put(key, ret);
			return( ret );
		}
	}
	...
	@Override
	public SymbolicSinh<R,S> cloneThread( final BigInteger threadIndex )
	{
		...
		{
			return( new SymbolicSinh<R,S>( elems , facs , ival ) );
		}
		return( this );
	}
	...
	public SymbolicElem<R,S> cloneThreadCached(
		...
		if( ( elems != elem ) || ( facs != fac ) )
		{
			final SymbolicSinh<R,S> rtmp = new SymbolicSinh<R,S>( elems , facs , ival );
			cache.put(this, rtmp);
			return( rtmp );
		}
		cache.put(this, this);
	...
	public String writeDesc( WriteElemCache<SymbolicElem<R,S>,SymbolicElemFactory<R,S>> cache , PrintStream ps )
	{
		...
		if( st == null )
		{
			...
			cache.put(this, st);
			ps.print( SymbolicSinh.class.getSimpleName() );
	...
	{
		ps.print( "<mrow><mi>sinh</mi><mo>&ApplyFunction;</mo>" );
