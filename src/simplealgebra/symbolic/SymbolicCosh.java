 ...
 */
public class SymbolicCosh<R extends Elem<R,?>, S extends ElemFactory<R,S>> extends SymbolicElem<R,S> 
{
	 ...
	 */
	public SymbolicCosh( SymbolicElem<R,S> _elem , S _fac , int _ival )
	{
		super( _fac );
		elem = _elem;
		ival = _ival;
	 ...
	 */
	public SymbolicCosh( SymbolicElem<R,S> _elem , S _fac , int _ival , DroolsSession ds )
	{
	...
	public R evalPartialDerivative( ArrayList<? extends Elem<?,?>> withRespectTo , HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ) throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		{
			final R cosV = elem.sinh( ival ).eval( implicitSpace );
			final R dv = elem.evalPartialDerivative( withRespectTo , implicitSpace );
			return( cosV.mult( dv ) );
		}
		else
		{
			return( super.cosh( ival ).evalPartialDerivative(withRespectTo, implicitSpace) );
		}
	}
			...
			HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace , HashMap<SCacheKey<R, S>, R> cache ) throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		if( ( this.getFac().isMultCommutative() ) && ( this.getFac().isMultAssociative() ) )
		{
			final R cosV = elem.sinh( ival ).evalCached( implicitSpace , cache );
		...
		{
			final R ret = super.cosh( ival ).evalPartialDerivativeCached(withRespectTo, implicitSpace, cache);
			cache.put(key, ret);
			return( ret );
		}
	}
	...
	@Override
	public SymbolicCosh<R,S> cloneThread( final BigInteger threadIndex )
	{
		...
		{
			return( new SymbolicCosh<R,S>( elems , facs , ival ) );
		}
		return( this );
	}
	...
	public SymbolicElem<R,S> cloneThreadCached(
		...
		if( ( elems != elem ) || ( facs != fac ) )
		{
			final SymbolicCosh<R,S> rtmp = new SymbolicCosh<R,S>( elems , facs , ival );
			cache.put(this, rtmp);
			return( rtmp );
		}
		cache.put(this, this);
	...
	public String writeDesc( WriteElemCache<SymbolicElem<R,S>,SymbolicElemFactory<R,S>> cache , PrintStream ps )
	{
		...
		if( st == null )
		{
			...
			cache.put(this, st);
			ps.print( SymbolicCosh.class.getSimpleName() );
	...
	{
		ps.print( "<mrow><mi>cosh</mi><mo>&ApplyFunction;</mo>" );
