...
/**
 * A symbolic elem for the left power function.
 * 
 ...
 */
public class SymbolicPowC<R extends Elem<R,?>, S extends ElemFactory<R,S>> extends SymbolicElem<R,S> 
{
	 ...
	 */
	public SymbolicPowC( SymbolicElem<R,S> _elemA , SymbolicElem<R,S> _elemB , S _fac , int _ivalExp , int _ivalLn )
	{
		super( _fac );
		elemA = _elemA;
		elemB = _elemB;
	 ...
	 */
	public SymbolicPowC( SymbolicElem<R,S> _elemA , SymbolicElem<R,S> _elemB , S _fac , int _ivalExp , int _ivalLn , DroolsSession ds )
	{
	...
	public R eval( HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ) throws NotInvertibleException, MultiplicativeDistributionRequiredException {
		return( elemA.eval( implicitSpace ).powC( elemB.eval( implicitSpace ) , ivalExp , ivalLn ) );
	}
	...
	public R evalCached( HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ,
		...
		final R ret = elemA.evalCached( implicitSpace , cache ).powC( elemB.evalCached( implicitSpace , cache ) , ivalExp , ivalLn );
		cache.put( key , ret );
		return( ret );
	}
	...
	public R evalPartialDerivative( ArrayList<? extends Elem<?,?>> withRespectTo , HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ) throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		{
			SymbolicElem<R,S> derivTermR = ( elemA.ln(ivalExp, ivalLn) ).mult( elemB );
			SymbolicElem<R,S> derivTermL = elemB.mult( elemA.ln(ivalExp, ivalLn) );
			final R expV = elemA.eval( implicitSpace ).powC( elemB.eval( implicitSpace ) , ivalExp , ivalLn );
			final R dvL = derivTermL.evalPartialDerivative( withRespectTo , implicitSpace );
			final R dvR = derivTermR.evalPartialDerivative( withRespectTo , implicitSpace );
			return( ( ( expV.mult( dvL ) ).add( expV.mult( dvR ) ) ).divideBy( 2 ) );
		}
		else
		{
			return( super.powC( elemB , ivalExp , ivalLn ).evalPartialDerivative(withRespectTo, implicitSpace) );
		}
	}
			...
			HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace , HashMap<SCacheKey<R, S>, R> cache ) throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		if( ( this.getFac().isMultCommutative() ) && ( this.getFac().isMultAssociative() ) )
		{
			SymbolicElem<R,S> derivTermR = ( elemA.ln(ivalExp, ivalLn) ).mult( elemB );
			SymbolicElem<R,S> derivTermL = elemB.mult( elemA.ln(ivalExp, ivalLn) );
			final R expV = elemA.evalCached( implicitSpace , cache ).powC( elemB.evalCached( implicitSpace , cache ) , ivalExp , ivalLn );
			final R dvL = derivTermL.evalPartialDerivativeCached( withRespectTo , implicitSpace , cache );
			final R dvR = derivTermR.evalPartialDerivativeCached( withRespectTo , implicitSpace , cache );
			final R ret = ( ( expV.mult( dvL ) ).add( expV.mult( dvR ) ) ).divideBy( 2 );
			cache.put(key, ret);
			return( ret );
		}
		else
		{
			final R ret = super.powC( elemB , ivalExp , ivalLn ).evalPartialDerivativeCached(withRespectTo, implicitSpace, cache);
			cache.put(key, ret);
			return( ret );
		}
	}
	...
	@Override
	public SymbolicPowC<R,S> cloneThread( final BigInteger threadIndex )
	{
		...
		{
			return( new SymbolicPowC<R,S>( elemsA , elemsB , facs , ivalExp , ivalLn ) );
		}
		return( this );
	}
	...
	public SymbolicElem<R,S> cloneThreadCached(
		...
		if( ( elemsA != elemA ) || ( elemsB != elemB ) || ( facs != fac ) )
		{
			final SymbolicPowC<R,S> rtmp = new SymbolicPowC<R,S>( elemsA , elemsB , facs , ivalExp , ivalLn );
			cache.put(this, rtmp);
			return( rtmp );
		}
		cache.put(this, this);
	...
	public String writeDesc( WriteElemCache<SymbolicElem<R,S>,SymbolicElemFactory<R,S>> cache , PrintStream ps )
	{
		...
		if( st == null )
		{
			...
			cache.put(this, st);
			ps.print( SymbolicPowC.class.getSimpleName() );
	...
	{
		ps.print( "<mrow><mi>powc</mi><mo>&ApplyFunction;</mo>" );
