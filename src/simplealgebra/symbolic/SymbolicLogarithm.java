...
/**
 * A symbolic elem for the natural logarithm function.
 * 
 ...
 */
public class SymbolicLogarithm<R extends Elem<R,?>, S extends ElemFactory<R,S>> extends SymbolicElem<R,S> 
{
	 ...
	 */
	public SymbolicLogarithm( SymbolicElem<R,S> _elem , S _fac , int _ivalExp , int _ivalLn )
	{
		super( _fac );
		elem = _elem;
		ivalExp = _ivalExp;
	 ...
	 */
	public SymbolicLogarithm( SymbolicElem<R,S> _elem , S _fac , int _ivalExp , int _ivalLn , DroolsSession ds )
	{
		this( _elem , _fac , _ivalExp , _ivalLn );
		ds.insert( this );
	}
	...
	public R eval( HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ) throws NotInvertibleException, MultiplicativeDistributionRequiredException {
		return( elem.eval( implicitSpace ).ln( ivalExp , ivalLn ) );
	}
	...
	public R evalCached( HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ,
		...
		final R ret = elem.evalCached( implicitSpace , cache ).ln( ivalExp , ivalLn );
		cache.put( key , ret );
		return( ret );
	}
	...
	public R evalPartialDerivative( ArrayList<? extends Elem<?,?>> withRespectTo , HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace ) throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		{
			final R expV = ( elem.eval( implicitSpace ) ).invertLeft();
			final R dv = elem.evalPartialDerivative( withRespectTo , implicitSpace );
			return( expV.mult( dv ) );
		}
		else
		{
			return( super.ln( ivalExp , ivalLn ).evalPartialDerivative(withRespectTo, implicitSpace) );
		}
	}
			...
			HashMap<? extends Elem<?,?>,? extends Elem<?,?>> implicitSpace , HashMap<SCacheKey<R, S>, R> cache ) throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		if( ( this.getFac().isMultCommutative() ) && ( this.getFac().isMultAssociative() ) )
		{
			final R expV = ( elem.evalCached( implicitSpace , cache ) ).invertLeft();
		...
		{
			final R ret = super.ln( ivalExp , ivalLn ).evalPartialDerivativeCached(withRespectTo, implicitSpace, cache);
			cache.put(key, ret);
			return( ret );
		}
	}
	...
	@Override
	public SymbolicLogarithm<R,S> cloneThread( final BigInteger threadIndex )
	{
		...
		{
			return( new SymbolicLogarithm<R,S>( elems , facs , ivalExp , ivalLn ) );
		}
		return( this );
	}
	...
	public SymbolicElem<R,S> cloneThreadCached(
		...
		if( ( elems != elem ) || ( facs != fac ) )
		{
			final SymbolicLogarithm<R,S> rtmp = new SymbolicLogarithm<R,S>( elems , facs , ivalExp , ivalLn );
			cache.put(this, rtmp);
			return( rtmp );
		}
		cache.put(this, this);
	...
	public String writeDesc( WriteElemCache<SymbolicElem<R,S>,SymbolicElemFactory<R,S>> cache , PrintStream ps )
	{
		...
		if( st == null )
		{
			...
			cache.put(this, st);
			ps.print( SymbolicLogarithm.class.getSimpleName() );
	...
	{
		ps.print( "<mrow><mi>ln</mi><mo>&ApplyFunction;</mo>" );
