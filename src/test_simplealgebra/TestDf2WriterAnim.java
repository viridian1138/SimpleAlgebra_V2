...
public class TestDf2WriterAnim extends TestCase {
	 ...
	 */
	protected static final int NUM_T_ITER = 800; // IterConstants.LRG_ITER_T;
	
	/**
	 * The number of discretizations on the X-Axis.
	 */
	protected static final int NUM_X_ITER = 400; // IterConstants.LRG_ITER_X;
	
	/**
	 * The number of discretizations on the Y-Axis.
	 */
	protected static final int NUM_Y_ITER = 400; // IterConstants.LRG_ITER_Y;
	...
	public void testDf2FileWriter() throws Throwable
	{
		...
		DrFastArray3D_Dbl iterArrayIm = new DrFastArray3D_Dbl( dparam , databaseLocationIm );
		...
		for( int tval = 0 ; tval < NUM_T_ITER ; ++tval )
		{
			...
			for( int y = NUM_Y_ITER - 1 ; y >= 0 ; y-- )
			{
				...
				for( int x = 0 ; x < NUM_X_ITER ; x++ )
				{
					...
					int red = (int)( 255 * ( v + maxAbs ) / ( 2.0 * maxAbs ) );
					if( red > 255 ) red = 255;
					final int green = red;
					final int blue = 255;
