 ...
 *
 */
public class TestSchrodingerNC_DR_Ncore extends TestCase {
	 ...
	 */
	protected static TestSchrodingerNC_DR_Ncore.IterationThreadContext[] initIterationThreadContext()
	{
		final TestSchrodingerNC_DR_Ncore.IterationThreadContext[] ret = new TestSchrodingerNC_DR_Ncore.IterationThreadContext[ NUM_CPU_CORES ];
		for( int cnt = 0 ; cnt < NUM_CPU_CORES ; cnt++ )
		{
			ret[ cnt ] = new TestSchrodingerNC_DR_Ncore.IterationThreadContext();
		}
		return( ret );
	}
	 ...
	 */
	protected static TestSchrodingerNC_DR_Ncore.IterationThreadContext[] iterationThreadContexts = initIterationThreadContext();
	...
	protected static class StelemNewton extends NewtonRaphsonSingleElem<ComplexElem<DoubleElem,DoubleElemFactory>,ComplexElemFactory<DoubleElem,DoubleElemFactory>>
	{
		...
		public StelemNewton(
			...
			threadContext = TestSchrodingerNC_DR_Ncore.iterationThreadContexts[ threadIndex ];
		}
		...
		protected StelemNewton( final StelemNewton in , final BigInteger threadIndex )
		{
			super( in , threadIndex );
			final int threadInd = threadIndex.intValue();
			threadContext = TestSchrodingerNC_DR_Ncore.iterationThreadContexts[ threadInd ];
		}
			...
			throws NotInvertibleException, MultiplicativeDistributionRequiredException, Throwable
	{
		...
		for( int ccnt = 0 ; ccnt < NUM_CPU_CORES ; ccnt++ )
		{
			...
			final TestSchrodingerNC_DR_Ncore.IterationThreadContext threadContext = iterationThreadContexts[ core ];
	...
	public void testStelemSimple() throws Throwable
	{
		...
		{
			TestSchrodingerNC_DR_Ncore.iterationThreadContexts[ hcnt ].iterArrayRe = new DrFastArray4D_Dbl( dparam , databaseLocationRe );
			TestSchrodingerNC_DR_Ncore.iterationThreadContexts[ hcnt ].iterArrayIm = new DrFastArray4D_Dbl( dparam , databaseLocationIm );
		}
		...
		// System.out.println( "==============================" ); !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		// System.out.println( TestSchrodingerNC_DR_Ncore.iterationThreadContexts[ 0 ].iterArray[ NUM_T_ITER - 1 ][ HALF-X ][ HALF_Y ][ HALF_Z ] ); !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		// Assert.assertTrue( Math.abs( val - ( -1.450868 ) ) < 0.01 ); !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		
		
		for( int hcnt = 0 ; hcnt < NUM_CPU_CORES ; hcnt++ )
		{
			TestSchrodingerNC_DR_Ncore.iterationThreadContexts[ hcnt ].iterArrayRe.close();
			TestSchrodingerNC_DR_Ncore.iterationThreadContexts[ hcnt ].iterArrayRe.close();
		}
