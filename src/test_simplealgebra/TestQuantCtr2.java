 ...
 * 
 */
public class TestQuantCtr2 extends TestCase {

	public TestQuantCtr2(String name) {
		super(name);
	}
	 ...
	 */
	private static final double DSSX = 0.16;
	
	/**
	 * The phase rate of the initial conditions in radians over the full NUM_T_ITER_B.
	 */
	...
	protected static final int NUM_T_ITER_B = 2000;
	...
	final static double[] ctrRe = new double[ NUM_T_ITER_B ];
			
			
	final static double[] ctrIm = new double[ NUM_T_ITER_B ];
	
	
	final static double[] e2a = new double[ NUM_T_ITER_B ];
	...
	public void testQuantCtr2() throws Throwable
	{
		System.out.println( "Initializing ctr..." );
		final double IMMULT = 11.0;
		double startRe = 1.0;
		double startIm = 0.0;
		...
		while( !done )
		{
			...
			for( int tv = 0 ; tv < NUM_T_ITER_B ; tv++ )
			{
				double avRep = IMMULT * DSSX * - Math.sin( ERATE * tv / NUM_T_ITER_B + EPHASEB ) * ERATE / NUM_T_ITER_B;
				double avImp = DSSX * Math.cos( ERATE * tv / NUM_T_ITER_B + EPHASEB ) * ERATE / NUM_T_ITER_B;
				...
				ComplexElem<DoubleElem,DoubleElemFactory> sdv = prev.add( eval.mult( prev ) );
				...
				prevRe = ctrRe[ tv ];
				prevIm = ctrIm[ tv ];
				ComplexElem<DoubleElem,DoubleElemFactory> e1 = II.mult( eval ).negate();
				e2a[ tv ] = ( e1.mult( e1 ) ).getRe().getVal();
			}
		}
		...
		for( int tv = 0 ; tv < NUM_T_ITER_B ; tv++ )
		{
			ctrRe[ tv ] = ctrRe[ tv ] * 5000.0;
			ctrIm[ tv ] = ctrIm[ tv ] * 5000.0;
		}
		...
		for( int tv = 0 ; tv < NUM_T_ITER_B ; tv++ )
		{
			...
			System.out.println( tv + " " + mag + " " + e2a[ tv ] );
		}
