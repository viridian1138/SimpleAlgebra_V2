...
public class TestConnectionCoefficient extends TestCase {
	...
	public void testConnectionCoefficient() throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		final ConnectionCoefficientFactory<String, TestDimensionFour, DoubleElem, DoubleElemFactory, Ordinate> ofac =
				new ConnectionCoefficientFactory<String, TestDimensionFour, DoubleElem, DoubleElemFactory, Ordinate>(
						tmt, new TestTemporaryIndexFactory(), ofacI);
		...
		final SymbolicElem<EinsteinTensorElem<String, SymbolicElem<DoubleElem, DoubleElemFactory>, 
			...
			odir = ofac.getConnectionCoefficient("alpha", "beta", "u");
		...
		System.out.println( "=======" );
		its = ev.getCovariantIndices().iterator();
		while( its.hasNext() )
		{
			System.out.println( its.next() );
		}
