 ...
 *
 */
public class TestQuantB2_NC_DR_Ncore extends TestCase {
	 ...
	 */
	protected static TestQuantB2_NC_DR_Ncore.IterationThreadContext[] initIterationThreadContext()
	{
		final TestQuantB2_NC_DR_Ncore.IterationThreadContext[] ret = new TestQuantB2_NC_DR_Ncore.IterationThreadContext[ NUM_CPU_CORES ];
		for( int cnt = 0 ; cnt < NUM_CPU_CORES ; cnt++ )
		{
			ret[ cnt ] = new TestQuantB2_NC_DR_Ncore.IterationThreadContext();
		}
		return( ret );
	}
	 ...
	 */
	protected static TestQuantB2_NC_DR_Ncore.IterationThreadContext[] iterationThreadContexts = initIterationThreadContext();
	...
	private static class BVelem extends Nelem<ComplexElem<DoubleElem,DoubleElemFactory>,ComplexElemFactory<DoubleElem,DoubleElemFactory>,Ordinate>
	{
		 ...
		 */
		protected TestQuantB2_NC_DR_Ncore.IterationThreadContext threadContext;
...
//			return( new ComplexElem<DoubleElem,DoubleElemFactory>( re , new DoubleElem( 0.0 ) ) );
	...
	private static class BAelem extends Nelem<ComplexElem<DoubleElem,DoubleElemFactory>,ComplexElemFactory<DoubleElem,DoubleElemFactory>,Ordinate>
	{
		...
		public ComplexElem<DoubleElem,DoubleElemFactory> eval(HashMap<? extends Elem<?, ?>, ? extends Elem<?, ?>> implicitSpace)
			...
			TestQuantB2_NC_DR_Ncore.IterationThreadContext threadContext = iterationThreadContexts[ threadI ];
	...
	protected static class StelemNewton extends NewtonRaphsonSingleElemCompiled<ComplexElem<DoubleElem,DoubleElemFactory>,ComplexElemFactory<DoubleElem,DoubleElemFactory>>
	{
		...
		public StelemNewton(
			...
			threadContext = TestQuantB2_NC_DR_Ncore.iterationThreadContexts[ threadIndex ];
		}
		...
		protected StelemNewton( final StelemNewton in , final BigInteger threadIndex )
		{
			super( in , threadIndex );
			final int threadInd = threadIndex.intValue();
			threadContext = TestQuantB2_NC_DR_Ncore.iterationThreadContexts[ threadInd ];
		}
	...
	protected void initIterArray( final double d1 ) throws Throwable
	{
		...
		final TestQuantB2_NC_DR_Ncore.IterationThreadContext iterContext = iterationThreadContexts[ 0 ];
		...
		for( int tcnt = 0 ; tcnt < 2 * NSTPT ; tcnt++ )
		{
			...
			for( long acnt = 0 ; acnt < ( (long) NUM_X_ITER ) * NUM_Y_ITER * NUM_Z_ITER ; acnt++ )
			{
				...
				else
				{
					...
					if( dx * dx + dy * dy + dz * dz < 1.0 )
					{
						iterArrayRe.set( tcnt , x , y , z , ctrRe[ tcnt ] );
						iterArrayIm.set( tcnt , x , y , z , ctrIm[ tcnt ] );
					}
					else
					{
			...
			throws NotInvertibleException, MultiplicativeDistributionRequiredException, Throwable
	{
		...
		for( int ccnt = 0 ; ccnt < NUM_CPU_CORES ; ccnt++ )
		{
			...
			final TestQuantB2_NC_DR_Ncore.IterationThreadContext threadContext = iterationThreadContexts[ core ];
				...
				public void run()
				{
					try
					{
						...
						while( !( im.isDone() ) )
						{
							...
							{
								threadContext.resetCorrectionValueRe( ctrRe[ tval ] );
								threadContext.resetCorrectionValueIm( ctrIm[ tval ] );
								threadContext.cacheIterationValue();
							}
							else
							{
	...
	{
		TestQuantB2_NC_DR_Ncore.initCtr();
		...
		{
			TestQuantB2_NC_DR_Ncore.iterationThreadContexts[ hcnt ].iterArrayRe = new DrFastArray4D_Dbl( dparam , databaseLocationRe );
			TestQuantB2_NC_DR_Ncore.iterationThreadContexts[ hcnt ].iterArrayIm = new DrFastArray4D_Dbl( dparam , databaseLocationIm );
		}
		...
		// System.out.println( "==============================" ); !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		// System.out.println( TestQuantB2_NC_DR_Ncore.iterationThreadContexts[ 0 ].iterArray[ NUM_T_ITER - 1 ][ HALF-X ][ HALF_Y ][ HALF_Z ] ); !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		// Assert.assertTrue( Math.abs( val - ( -1.450868 ) ) < 0.01 ); !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		
		
		for( int hcnt = 0 ; hcnt < NUM_CPU_CORES ; hcnt++ )
		{
			TestQuantB2_NC_DR_Ncore.iterationThreadContexts[ hcnt ].iterArrayRe.close();
			TestQuantB2_NC_DR_Ncore.iterationThreadContexts[ hcnt ].iterArrayRe.close();
		}
