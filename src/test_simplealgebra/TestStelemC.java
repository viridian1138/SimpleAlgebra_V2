...
public class TestStelemC extends TestCase {
	...
	protected static class StelemNewton extends NewtonRaphsonSingleElem<DoubleElem,DoubleElemFactory>
	{
		...
		{
			TestStelemC.cacheIterationValue();
		}
		
		@Override
		protected void retrieveIterationValue()
		{
			TestStelemC.retrieveIterationValue();
		}
	...
	protected void initIterArray( final double d1 )
	{
		...
		for( int tcnt = 0 ; tcnt < 2 * NSTPT ; tcnt++ )
		{
			...
			for( long acnt = 0 ; acnt < ( (long) NUM_X_ITER ) * NUM_Y_ITER ; acnt++ )
			{
				...
				if( dx * dx + dy * dy < 1.0 )
				{
					iterArray[ tcnt ][ x ][ y ] = 10000.0 * ( d1 * d1 );
				}
				else
				{
					iterArray[ tcnt ][ x ][ y ] = 0.0;
				}
			}
		}
	...
	public void testStelemSimple() throws NotInvertibleException, MultiplicativeDistributionRequiredException
	{
		...
		System.out.println( "==============================" );
		System.out.println( iterArray[ NUM_T_ITER - 1 ][ HALF_X ][ HALF_Y ] );
