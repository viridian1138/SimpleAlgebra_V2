 ...
 *
 */
public class TestSchrodingerNA extends TestCase {
	...
	private static class BNelem extends Nelem<ComplexElem<DoubleElem,DoubleElemFactory>,ComplexElemFactory<DoubleElem,DoubleElemFactory>,Ordinate>
	{
		...
		public ComplexElem<DoubleElem,DoubleElemFactory> eval(HashMap<? extends Elem<?, ?>, ? extends Elem<?, ?>> implicitSpace)
			...
			final DoubleElem re = new DoubleElem( TestSchrodingerNA.tempArrayRe[ cols[ 0 ] ][ cols[ 1 ] ][ cols[ 2 ] ][ cols[ 3 ] ] );
			final DoubleElem im = new DoubleElem( TestSchrodingerNA.tempArrayIm[ cols[ 0 ] ][ cols[ 1 ] ][ cols[ 2 ] ][ cols[ 3 ] ] );
	...
	protected static class StelemNewton extends NewtonRaphsonSingleElem<ComplexElem<DoubleElem,DoubleElemFactory>,ComplexElemFactory<DoubleElem,DoubleElemFactory>>
	{
		...
		{
			TestSchrodingerNA.performIterationUpdate( iterationOffset );
		}
		
		@Override
		protected void cacheIterationValue()
		{
			TestSchrodingerNA.cacheIterationValue();
		}
		
		@Override
		protected void retrieveIterationValue()
		{
			TestSchrodingerNA.retrieveIterationValue();
		}
		...
		for( long acnt = 0 ; acnt < ( ( (long) NUM_X_ITER ) * NUM_Y_ITER * NUM_Z_ITER ) ; acnt++ )
		{
			...
			final ComplexElem<DoubleElem,DoubleElemFactory> ival =
					new ComplexElem<DoubleElem,DoubleElemFactory>(
							new DoubleElem( TestSchrodingerNA.getUpdateValueRe() ) , 
							new DoubleElem( TestSchrodingerNA.getUpdateValueIm() ) );
	...
	protected void initIterArray( final double d1 )
	{
		...
		for( int tcnt = 0 ; tcnt < 2 * NSTPT ; tcnt++ )
		{
			...
			for( long acnt = 0 ; acnt < ( (long) NUM_X_ITER ) * NUM_Y_ITER * NUM_Z_ITER ; acnt++ )
			{
				...
				{
					iterArrayRe[ tcnt ][ x ][ y ][ z ] = 0.0;
				}
				iterArrayIm[ tcnt ][ x ][ y ][ z ] = 0.0;
			}
		}
